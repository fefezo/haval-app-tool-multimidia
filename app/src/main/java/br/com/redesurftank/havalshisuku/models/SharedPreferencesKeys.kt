...
enum class SharedPreferencesKeys(val key: String, val description: String) {
    ...
    SET_STARTUP_AC("setStartupAc", "Definir ar-condicionado ao ligar o veículo"),
    ...
    STARTUP_AC_POWER("startupAcPower", "Ligar o A/C ao ligar o veículo (falso = só ajusta, sem ligar)"),
    STARTUP_AC_COMPRESSOR("startupAcCompressor", "Compressor do A/C ao ligar o veículo (só vale quando liga o A/C)"),
    DEFAULT_AC_TEMPERATURE_DRIVER("defaultAcDriverTemperature", "Temperatura do modo padrão do A/C (lado do motorista)"),
    DEFAULT_AC_TEMPERATURE_PASSENGER("defaultAcPassengerTemperature", "Temperatura do modo padrão do A/C (lado do passageiro)"),
    DEFAULT_AC_FAN_SPEED("defaultAcFanSpeed", "Velocidade da ventilação do modo padrão do A/C"),
    DEFAULT_AC_CYCLE_MODE("defaultAcCycleMode", "Circulação do ar do modo padrão do A/C"),
    DEFAULT_AC_BLOWER_MODE("defaultAcBlowerMode", "Direção do ar do modo padrão do A/C (vazio = não alterar)"),
    DEFAULT_AC_COMPRESSOR("defaultAcCompressor", "Compressor do A/C no modo padrão do A/C"),
    ...
    LAST_CLUSTER_FONT("lastClusterFont", "Fonte da interface do cluster"),
